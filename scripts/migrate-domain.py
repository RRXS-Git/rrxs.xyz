...
for p in projects:
    ...
    try:
        ...
        for d_domain in dr.get('result', []):
            dn = d_domain.get('name','')
            if dn == 'rrxs.xyz':
                old_project = name
                print(f'  >>> rrxs.xyz FOUND on project: {name}')
    except Exception as e:
        print(f'  {name}: domain check error - {type(e).__name__}')
...
try:
    ...
    for dep in dr.get('result', [])[:5]:
        print(f'  {dep.get("id","?")[:12]}: created={dep.get("created_on","?")} stage={dep.get("stage","?")} branch={dep.get("deployment_trigger",{}).get("branch","?")} canonical={dep.get("is_canonical",False)}')
except Exception as e:
...
try:
    ...
    for d_domain in dr.get('result', []):
        print(f'  {d_domain.get("name")} status={d_domain.get("status")}')
except Exception as e:
